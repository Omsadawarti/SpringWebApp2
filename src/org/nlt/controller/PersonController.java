...
public class PersonController extends ParentController {
	...
	public ModelAndView updatePerson(HttpServletRequest req, HttpServletResponse resp) {
		...
		String id = req.getParameter("id");
		Persons person = personService.getPerson(Integer.parseInt(id));//will fetch existing object from database
		if (person != null) {
			...
			if(result)
			{
				m.put("SuccessMessage", "Record Updated Successfully");
				m.put("NameValue", "");
				m.put("AgeValue", "");
				m.put("PhoneValue", "");
			...
			{
				m.put("ErrorMessage", "Couldn't Update Record!");
			}
		}
		m.put("Action", "./updatePerson");
		m.put("ButtonValue", "Update");
